...
class MyDocumentsHandler {
    ...
    addItemToDocuments = async () => {

    }

    removeItemsFromDocuments = async () => {
    ...
    downloadItem = async (item, path, cookies) => {
        ...
        downloadStream
            .on("downloadProgress", ({ transferred, total, percent }) => {
                ...
                console.error(`downloading ${item.name}: ${transferred}/${total} - (${percentage}%)`);
            })
            .on("error", (error) => {
        ...
        fileWriterStream
            ...
            .on("finish", () => {
                console.log(`File downloaded to ${item.name}`);
            });
